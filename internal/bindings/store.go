	...
	_, err = s.db.ExecContext(ctx, `
        INSERT INTO bindings
            (machine_uuid, image_id, profile_id, desired_state,
             ...
             bond_override, updated_at, updated_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		muuid, imageID, profileID, desired, addrSQL, hostnameSQL, ctSQL, tdSQL, bondSQL, now, in.UpdatedBy,
	)
	if err != nil {
...
func (s *Store) Get(ctx context.Context, machineUUID string) (*Binding, error) {
	...
	var b Binding
	var addr, hostname, tdJSON, bondJSON sql.NullString
	var updatedAt int64
	var passwordCT []byte
	err = s.db.QueryRowContext(ctx, `
		...
		&addr, &hostname, &passwordCT, &tdJSON, &bondJSON, &updatedAt, &b.UpdatedBy,
	)
...
func (s *Store) List(ctx context.Context) ([]Binding, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.MachineUUID, &b.ImageID, &b.ProfileID, &b.DesiredState,
			&addr, &hostname, &passwordCT, &tdJSON, &bondJSON, &updatedAt, &b.UpdatedBy); err != nil {
