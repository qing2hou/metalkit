...
(function () {
  ...
  function renderForm(existing) {
    ...
    return (
      ...
      '<div class="form-row"><label for="f-pass">Root 密码' +
      (existing ? '' : ' <span class="required">*</span>') + '</label>' +
      '<div class="inline-row">' +
      '<input type="password" id="f-pass" autocomplete="new-password" placeholder="' +
      (existing ? "留空保持不变" : "至少 8 字符") + '">' +
      ...
      '<div class="muted">服务端使用 SHA-512 crypt 哈希，明文不会保存。</div></div>' +
      ...
      '<div class="muted">802.1Q VLAN tag（1–4094）。启用后 IP 配置会上移到 VLAN 子接口。</div></div>' +
  ...
  function wireFormBehaviour(dlg, existing) {
    ...
    netMethod.addEventListener("change", function () {
      netStatic.hidden = (netMethod.value !== "static");
    });
  ...
  async function submitForm(dlg, existing) {
    ...
    if (!isEdit && !passHash) {
      throw new Error("新建配置时 Root 密码必填");
    }
  ...
  function openNICPicker(opts) {
    ...
    MK.apiGet("/machines").then(function (machines) {
      ...
      dlg.querySelector("#picker-error").textContent = "加载机器列表失败：" + err.message;
      dlg.querySelector("#picker-error").hidden = false;
    });
  ...
  function openDiskPicker(opts) {
    ...
    MK.apiGet("/machines").then(function (machines) {
      ...
      dlg.querySelector("#dpicker-error").textContent = "加载机器列表失败：" + err.message;
      dlg.querySelector("#dpicker-error").hidden = false;
    });
